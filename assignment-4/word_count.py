...
# MAGIC %md ** (1b) Capitalize and test **
# MAGIC 
...
# MAGIC %md ** (1c) Apply `capitalize` to the base RDD **
# MAGIC 
...
# MAGIC %md ** (1d) Pass a `lambda` function to `map` **
# MAGIC 
...
# MAGIC %md ** (1e) Length of each word **
# MAGIC 
...
# MAGIC %md #### ** Part 2: Counting with Pair RDDs **
# MAGIC 
...
# MAGIC %md ** (2b) Use `groupByKey()` to obtain the counts **
# MAGIC 
...
# TODO: Replace <FILL IN> with appropriate code
wordCountsGrouped = wordsGrouped.map(lambda (k, it_v): (k, sum(it_v)))
...
# MAGIC %md ** (2c) Count using `reduceByKey` **
# MAGIC 
...
# MAGIC %md #### ** Part 3: More Transformations **
# MAGIC 
...
# MAGIC %md ** (3b) Calculate mean using `reduce` **
# MAGIC 
...
# MAGIC %md #### ** Part 4: Applying to a File **
# MAGIC 
...
# MAGIC %md ** (4b) Capitalization and punctuation **
# MAGIC 
...
def removePunctuation(text):
    ...
    """
    return re.sub('[^A-Za-z0-9 ]+', '', text.lower().lstrip().rstrip())
...
# TODO: Replace <FILL IN> with appropriate code
top10WordsAndCounts = wordCount(shakespeareWordsRDD).takeOrdered(10, lambda (k, v): -v)
