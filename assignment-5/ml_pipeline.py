...
# MAGIC 
# MAGIC **TODO**: Read the documentation and examples for [Spark Machine Learning Pipeline](https://spark.apache.org/docs/1.6.2/ml-guide.html#main-concepts-in-pipelines).

# COMMAND ----------

# MAGIC %md ## Part 1: Business Understanding
...
# MAGIC %md ## Part 2: Extract-Transform-Load (ETL) Your Data
# MAGIC 
...
# MAGIC 
# MAGIC 
# MAGIC **TODO**: Let's start by printing a sample of the data.
# MAGIC 
...
# MAGIC %md Next, use the `dbutils.fs.head` command to look at the first 65,536 bytes of the first file in the directory.
# MAGIC 
...
# MAGIC %md `dbutils.fs` has its own help facility, which we can use to see the various available functions.
...
# MAGIC %md From our initial exploration of a sample of the data, we can make several observations for the ETL process:
...
# MAGIC ### Exercise 2(b)
# MAGIC 
# MAGIC **TODO**: Create a DataFrame from the data.
# MAGIC 
...
# MAGIC %md Check the names and types of the columns using the [dtypes](https://spark.apache.org/docs/1.6.2/api/python/pyspark.sql.html#pyspark.sql.DataFrame.dtypes) method.
...
# MAGIC %md We can examine the data using the display() method.
...
# MAGIC %md ### Part 2: Alternative Method to Load your Data
...
altPlantDF = sqlContext.read.format('com.databricks.spark.csv').options(delimiter='\t', header='true').load("/databricks-datasets/power-plant/data", schema=customSchema)
...
# MAGIC %md Now we can examine the data using the display() method. *Note that this operation will cause the data to be read and the DataFrame will be created.*
...
# MAGIC %md ## Part 3: Explore Your Data
...
# MAGIC %md First, let's register our DataFrame as an SQL table named `plant_dataset`.  Because you may run this lab multiple times, we'll take the precaution of removing any existing tables first.
# MAGIC 
# MAGIC We can delete any existing `plant_dataset` SQL table using the SQL command: `DROP TABLE IF EXISTS plant_dataset` (we also need to to delete any Hive data associated with the table, which we can do with a Databricks file system operation).
# MAGIC 
...
# MAGIC %md Now that our DataFrame exists as a SQL table, we can explore it using SQL commands. This [mini tutorial](https://tutorialzine.com/2016/01/learn-sql-in-20-minutes) could help refeshing the SQL knowledge, and the top three sections therein are enough for this lab.
# MAGIC 
...
# MAGIC 
# MAGIC `%sql SELECT * FROM plant_dataset`
# MAGIC 
# MAGIC `display(sqlContext.sql("SELECT * FROM plant_dataset"))`
# MAGIC 
# MAGIC ### 3(b)
# MAGIC 
# MAGIC **TODO**: Execute the prepared code in the following cell.
...
# MAGIC SELECT * FROM plant_dataset
...
# MAGIC %md Let's perform some basic statistical analyses of all the columns.
# MAGIC 
...
# MAGIC %md ##Part 4: Visualize Your Data
# MAGIC 
...
# MAGIC 
# MAGIC <img src="https://raw.githubusercontent.com/hkust-comp4651-2018f/assets/master/assignment-5/customize_plot_small.png" style="border: 1px solid #999999"/>
# MAGIC 
...
# MAGIC %sql
# MAGIC select AT as AtmosphericTemperature, PE as PowerOutput from plant_dataset
...
# MAGIC %md ### Exercise 4(b)
# MAGIC 
# MAGIC Use SQL to create a scatter plot of PowerOutput(PE) as a function of VacuumSpeed (V).
# MAGIC Name the y-axis "PowerOutput" and the x-axis "VacuumSpeed"

# COMMAND ----------

# MAGIC %sql
# MAGIC -- TODO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC SELECT PE AS PowerOutput, V AS VacuumSpeed FROM plant_dataset

# COMMAND ----------

# MAGIC %md Let's continue exploring the relationships (if any) between the variables and Power Output.
# MAGIC 
# MAGIC ### Exercise 4(c)
# MAGIC 
# MAGIC Use SQL to create a scatter plot of PowerOutput(PE) as a function of AtmosphericPressure (AP).
# MAGIC Name the y-axis "PowerOutput" and the x-axis "AtmosphericPressure"

# COMMAND ----------

# MAGIC %sql
# MAGIC -- TODO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC SELECT PE AS PowerOutput, AP AS AtmosphericPressure FROM plant_dataset

# COMMAND ----------

# MAGIC %md ### Exercise 4(d)
# MAGIC 
# MAGIC Use SQL to create a scatter plot of PowerOutput(PE) as a function of RelativeHumidity(RH).
# MAGIC Name the y-axis "PowerOutput" and the x-axis "RelativeHumidity"

# COMMAND ----------

# MAGIC %sql
# MAGIC -- TODO: Replace <FILL_IN> with the appropriate SQL command.
# MAGIC SELECT RH AS RelativeHumidity, PE AS PowerOutput FROM plant_dataset

# COMMAND ----------

# MAGIC %md ##Part 5: Data Preparation
# MAGIC 
...
# MAGIC - Convert the `plant_dataset` SQL table into a DataFrame.
...
# MAGIC %md Next we'll create a Linear Regression Model and use the built in help to identify how to train it. See API details for [Linear Regression](https://spark.apache.org/docs/1.6.2/api/python/pyspark.ml.html#pyspark.ml.regression.LinearRegression) in the ML guide.
# MAGIC 
# MAGIC ### Exercise 6(b)
# MAGIC 
...
# MAGIC %md The cell below is based on the [Spark ML Pipeline API for Linear Regression](https://spark.apache.org/docs/1.6.2/api/python/pyspark.ml.html#pyspark.ml.regression.LinearRegression).
# MAGIC 
# MAGIC The first step is to set the parameters for the method:
# MAGIC - Set the name of the prediction column to "Prediction_PE"
# MAGIC - Set the name of the label column to "PE"
# MAGIC - Set the maximum number of iterations to 100
# MAGIC - Set the regularization parameter to 0.15
# MAGIC 
...
# MAGIC %md From the Wikipedia article on [Linear Regression](https://en.wikipedia.org/wiki/Linear_regression):
...
# MAGIC %md Recall **Part 4: Visualize Your Data** when we visualized each predictor against Power Output using a Scatter Plot, does the final equation seems logical given those visualizations?
# MAGIC 
# MAGIC ### Exercise 6(e)
# MAGIC 
# MAGIC Now let's see what our predictions look like given this model. We apply our Linear Regression model to the 15% of the data that we split from the input dataset. The output of the model will be a predicted Power Output column named "Prediction_PE".
# MAGIC 
# MAGIC - Run the next cell
# MAGIC - Scroll through the resulting table and notice how the values in the Power Output (PE) column compare to the corresponding values in the predicted Power Output (Prediction_PE) column
...
# MAGIC %md From a visual inspection of the predictions, we can see that they are close to the actual values.
# MAGIC 
...
# MAGIC %md Another useful statistical evaluation metric is the coefficient of determination, denoted \\(R^2\\) or \\(r^2\\) and pronounced "R squared". It is a number that indicates the proportion of the variance in the dependent variable that is predictable from the independent variable and it provides a measure of how well observed outcomes are replicated by the model, based on the proportion of total variation of outcomes explained by the model. The coefficient of determination ranges from 0 to 1 (closer to 1), and the higher the value, the better our model.
# MAGIC 
...
# MAGIC %md Generally, assuming a Gaussian distribution of errors, a good model will have 68% of predictions within 1 RMSE and 95% within 2 RMSE of the actual value (see http://statweb.stanford.edu/~susan/courses/s60/split/node60.html).
# MAGIC 
# MAGIC Let's examine the predictions and see if a RMSE of 4.60 meets this criteria.
# MAGIC 
...
# MAGIC %md We can use SQL to explore the `Plant_RMSE_Evaluation` table. First let's look at at the table using a SQL SELECT statement.
# MAGIC 
# MAGIC ### Exercise 6(i)
# MAGIC 
# MAGIC Run the next cell and ensure that you understand what's going on.
...
# MAGIC %sql
# MAGIC SELECT * from Plant_RMSE_Evaluation

# COMMAND ----------

# MAGIC %md Now we can display the RMSE as a Histogram.
# MAGIC 
# MAGIC ### Exercise 6(j)
# MAGIC 
# MAGIC Perform the following steps:
...
# MAGIC SELECT Within_RSME from Plant_RMSE_Evaluation

# COMMAND ----------

# MAGIC %md Using a complex SQL SELECT statement, we can count the number of predictions within + or - 1.0 and + or - 2.0 and then display the results as a pie chart.
# MAGIC 
# MAGIC ### Exercise 6(k)
# MAGIC 
# MAGIC Perform the following steps:
...
# MAGIC FROM Plant_RMSE_Evaluation
# MAGIC GROUP BY RSME_Multiple

# COMMAND ----------

# MAGIC %md From the pie chart, we can see that 69% of our test data predictions are within 1 RMSE of the actual values, and 97% (69% + 28%) of our test data predictions are within 2 RMSE. So the model is pretty decent. Let's see if we can tune the model to improve it further.

# COMMAND ----------

# MAGIC %md ##Part 7: Tuning and Evaluation
# MAGIC 
...
# MAGIC %md Now that we have tuned our Linear Regression model, let's see what the new RMSE and \\(r^2\\) values are versus our intial model.
# MAGIC 
# MAGIC ### Exercise 7(b)
# MAGIC 
...
# MAGIC %md So tuned linear regression model improves a little bit over our initial model. Let's look at the regularization parameter that the [CrossValidator](https://spark.apache.org/docs/1.6.2/ml-tuning.html#cross-validation) has selected.
# MAGIC 
# MAGIC Recall that the orginal regularization parameter we used was 0.015.
# MAGIC 
...
# MAGIC %md Given that the only linearly correlated variable is Temperature, it makes sense try another Machine Learning method such as [Decision Tree](https://en.wikipedia.org/wiki/Decision_tree_learning) to handle non-linear data and see if we can improve our model.
# MAGIC 
...
dt.setPredictionCol("Prediction_PE").setFeaturesCol("features").setLabelCol("PE").setMaxBins(100)
...
# MAGIC %md The line below will pull the Decision Tree model from the Pipeline as display it as an if-then-else string. Again, we have to "reach through" to the JVM API to make this one work.
# MAGIC 
...
# MAGIC   - Set the name of the prediction column to "Prediction_PE"
# MAGIC   - Set the name of the features column to "features"
# MAGIC   - Set the random number generator seed to 190088121L
# MAGIC   - Set the maximum depth to 8
# MAGIC   - Set the number of trees to 25
...
rf.setPredictionCol("Prediction_PE").setFeaturesCol("features").setSeed(190088121L).setMaxDepth(8).setNumTrees(25).setLabelCol("PE")
...
# MAGIC %md As with Decision Trees, instead guessing what parameters to use, we will use [Model Selection](https://spark.apache.org/docs/1.6.2/ml-tuning.html#model-selection-aka-hyperparameter-tuning) or [Hyperparameter Tuning](https://spark.apache.org/docs/1.6.2/ml-tuning.html#model-selection-aka-hyperparameter-tuning) to create the best model.
# MAGIC 
...
# MAGIC %md Note that the Decision Tree and Random Forest values for `r2` are identical. However, the RMSE for the Random Forest model is better.

# COMMAND ----------

# MAGIC %md The line below will pull the Random Forest model from the Pipeline as display it as an if-then-else string.
# MAGIC 
# MAGIC **ToDo**: Run the next cell
...
# MAGIC %md ### Conclusion
# MAGIC 
# MAGIC Wow! So our best model is in fact our Random Forest tree model which uses an ensemble of 25 Trees with a depth of 8 to construct a better model than the single decision tree.
